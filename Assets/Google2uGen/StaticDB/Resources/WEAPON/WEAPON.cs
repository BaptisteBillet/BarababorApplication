...
namespace Google2u
{
	...
	public class WEAPONRow : IGoogle2uRow
	{
		...
		public WEAPONRow(string __ID, string __Level, string __LevelUp, string __Name, string __Damage, string __DamageUpgrade, string __Range, string __RangeUpgrade, string __Cooldown, string __CooldownUpgrade, string __ShootType, string __ShootTypeUpgrade, string __Cost, string __Grade, string __Rank, string __State1, string __State2) 
		{
			...
			{
			int res;
				if(int.TryParse(__Cooldown, NumberStyles.Any, CultureInfo.InvariantCulture, out res))
					_Cooldown = res;
				else
					Debug.LogError("Failed To Convert _Cooldown string: "+ __Cooldown +" to int");
			}
			{
			int res;
				if(int.TryParse(__CooldownUpgrade, NumberStyles.Any, CultureInfo.InvariantCulture, out res))
					_CooldownUpgrade = res;
				else
					Debug.LogError("Failed To Convert _CooldownUpgrade string: "+ __CooldownUpgrade +" to int");
			}
			{
			int res;
